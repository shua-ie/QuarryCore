...
class MetadataExtractor:
    ...
    async def _extract_dates(self, metadata: ContentMetadata, html_content: str, url: str) -> None:
        ...
        try:
            date_info = await self.date_extractor.extract_publication_date(
                html_content, url
            )
    ...
    async def _enrich_medical_metadata(self, metadata: ContentMetadata, text_content: str) -> None:
        ...
        medical_indicators = {
            "has_medical_terms": any(term in text_content.lower() for term in 
                                   ["patient", "diagnosis", "treatment", "clinical", "medical"]),
            "has_citations": "pubmed" in text_content.lower() or "doi:" in text_content.lower(),
            "has_disclaimers": any(term in text_content.lower() for term in 
                                 ["disclaimer", "consult", "physician", "doctor"]),
        }
    ...
    async def _enrich_legal_metadata(self, metadata: ContentMetadata, text_content: str) -> None:
        ...
        legal_indicators = {
            "has_legal_citations": any(pattern in text_content for pattern in 
                                     ["v.", "F.2d", "F.3d", "U.S.C.", "C.F.R."]),
            "has_jurisdiction": any(term in text_content.lower() for term in 
                                  ["court", "federal", "state", "district"]),
    ...
    async def _enrich_ecommerce_metadata(self, metadata: ContentMetadata, html_content: str) -> None:
        ...
        ecommerce_indicators = {
            ...
            "has_reviews": any(term in html_content.lower() for term in 
                             ["review", "rating", "stars", "customer"]),
            "has_product_info": any(term in html_content.lower() for term in 
                                  ["sku", "model", "brand", "warranty"]),
        }
    ...
    async def _enrich_technical_metadata(self, metadata: ContentMetadata, text_content: str) -> None:
        ...
        technical_indicators = {
            "has_code_examples": any(term in text_content.lower() for term in 
                                   ["function", "class", "import", "def ", "var "]),
            "has_api_docs": any(term in text_content.lower() for term in 
                              ["api", "endpoint", "request", "response"]),
            "has_version_info": any(term in text_content for term in 
                                  ["v1.", "v2.", "version", "release"]),
        }
    ...
    async def extract_batch(
        ...
        *,
        ...
        max_workers = parallel_workers or (
            self.hardware_caps.recommended_workers if self.hardware_caps else 4
        )
