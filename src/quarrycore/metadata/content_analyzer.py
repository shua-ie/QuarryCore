...
class ContentAnalyzer:
    ...
    def __init__(self) -> None:
        ...
        self.category_keywords = {
            'technology': [
                ...
                'database', 'algorithm', 'machine learning', 'ai', 'artificial intelligence',
                'cloud', 'server', 'network', 'security', 'cyber', 'digital',
                'mobile', 'app', 'application', 'web', 'internet', 'data'
            ],
            'science': [
                'research', 'study', 'experiment', 'analysis', 'hypothesis',
                'theory', 'discovery', 'scientific', 'biology', 'chemistry',
                'physics', 'mathematics', 'statistics', 'laboratory', 'method'
            ],
            'business': [
                'market', 'company', 'business', 'industry', 'economy', 'finance',
                'investment', 'profit', 'revenue', 'sales', 'marketing', 'strategy',
                'management', 'corporate', 'enterprise', 'startup', 'entrepreneur'
            ],
            'health': [
                'health', 'medical', 'medicine', 'doctor', 'patient', 'treatment',
                'disease', 'symptom', 'therapy', 'hospital', 'clinical', 'diagnosis',
                'pharmaceutical', 'wellness', 'fitness', 'nutrition'
            ],
            'education': [
                'education', 'school', 'university', 'student', 'teacher', 'learning',
                'curriculum', 'academic', 'course', 'degree', 'training', 'knowledge',
                'skill', 'instruction', 'pedagogy'
            ],
            'news': [
                'news', 'report', 'journalist', 'media', 'press', 'breaking',
                'update', 'story', 'coverage', 'headline', 'article', 'newspaper',
                'broadcast', 'correspondent'
            ],
            'entertainment': [
                'entertainment', 'movie', 'film', 'music', 'game', 'sport',
                'celebrity', 'show', 'television', 'streaming', 'concert',
                'performance', 'artist', 'actor', 'musician'
            ],
            'lifestyle': [
                'lifestyle', 'fashion', 'travel', 'food', 'recipe', 'cooking',
                'home', 'garden', 'family', 'relationship', 'personal', 'hobby',
                'leisure', 'culture', 'art'
            ]
        }
        ...
        *,
        ...
        try:
            ...
            if calculate_reading_metrics:
                metrics.reading_metrics = await self._calculate_reading_metrics(
                    text_content, words
                )
            ...
            if analyze_lexical_diversity:
                metrics.lexical_metrics = await self._analyze_lexical_diversity(
                    text_content, words
                )
                metrics.lexical_diversity = metrics.lexical_metrics.lexical_diversity
            
            # Quality indicators
            metrics.quality_indicators = await self._analyze_quality_indicators(
    ...
    async def _analyze_html_structure_regex(self, html_content: str, indicators: QualityIndicators) -> None:
        ...
        # Check for meta elements
        indicators.has_meta_description = bool(
            re.search(r'<meta[^>]*name=["\']description["\']', html_content, re.IGNORECASE) or
            re.search(r'<meta[^>]*property=["\']og:description["\']', html_content, re.IGNORECASE)
        )
        
        # Check for structured data
        indicators.has_structured_data = bool(
            re.search(r'application/ld\+json', html_content, re.IGNORECASE) or
            re.search(r'itemscope', html_content, re.IGNORECASE)
        )
        
        # Check for author information
        indicators.has_author = bool(
            re.search(r'<meta[^>]*name=["\']author["\']', html_content, re.IGNORECASE) or
            re.search(r'rel=["\']author["\']', html_content, re.IGNORECASE) or
            re.search(r'class=["\'][^"\']*author[^"\']*["\']', html_content, re.IGNORECASE)
        )
        
        # Check for date information
        indicators.has_date = bool(
            re.search(r'<time[^>]*>', html_content, re.IGNORECASE) or
            re.search(r'article:published_time', html_content, re.IGNORECASE) or
            re.search(r'class=["\'][^"\']*date[^"\']*["\']', html_content, re.IGNORECASE)
        )
