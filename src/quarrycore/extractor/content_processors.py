...
class TableProcessor:
    ...
    async def _process_table(self, table_element: Any, table_index: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            table_data['structure']['column_count'] = len(headers) if headers else (
...
class CodeProcessor:
    ...
    def __init__(self) -> None:
        ...
        self.language_patterns: Dict[str, List[str]] = {
            'python': [
                r'\bdef\s+', r'\bclass\s+', r'\bimport\s+', r'\bfrom\s+.*import',
                r'print\s*\(', r'if\s+__name__\s*==\s*["\']__main__["\']',
            ],
            'javascript': [
                r'\bfunction\s+', r'\bvar\s+', r'\blet\s+', r'\bconst\s+',
                r'console\.log\s*\(', r'document\.', r'window\.',
            ],
            'java': [
                r'\bpublic\s+class\s+', r'\bpublic\s+static\s+void\s+main',
                r'\bSystem\.out\.', r'\bprivate\s+', r'\bprotected\s+',
            ],
            'c': [
            ...
            'sql': [
                r'\bSELECT\s+', r'\bFROM\s+', r'\bWHERE\s+', r'\bINSERT\s+',
                r'\bUPDATE\s+', r'\bDELETE\s+', r'\bCREATE\s+TABLE',
            ],
            'html': [
                r'<html[^>]*>', r'<head[^>]*>', r'<body[^>]*>', r'<div[^>]*>',
                r'<script[^>]*>', r'<style[^>]*>',
            ],
            'css': [
                r'\.[a-zA-Z][a-zA-Z0-9_-]*\s*{', r'#[a-zA-Z][a-zA-Z0-9_-]*\s*{',
...
class LinkProcessor:
    ...
    async def _process_link(
        ...
        try:
            ...
            link_data: Dict[str, Any] = {
                ...
                'category': await self._classify_link(link_element, href, link_text),  # Add category field for test compatibility
    ...
    async def _classify_link(self, link_element: Any, href: str, link_text: str) -> str:
        ...
        source_code_domains = ['github.com', 'gitlab.com', 'bitbucket.org', 'sourceforge.net']
