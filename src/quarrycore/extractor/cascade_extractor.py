...
class CascadeExtractor:
    ...
    def __init__(
        ...
        self.strategies: List[ExtractionStrategy] = [
            ...
            ExtractionStrategy("heuristic_fallback", priority=4, enabled=self.config.use_fallback_heuristics),
        ]
        ...
        *,
        ...
        try:
            ...
            if len(html_content) > self.config.max_content_length:
                result.warnings.append(f"Content truncated from {len(html_content)} to {self.config.max_content_length} chars")
            ...
            for strategy in sorted(self.strategies, key=lambda x: x.priority):
                ...
                try:
                    ...
                    strategy_result = await self._apply_extraction_strategy(
                        strategy, html_content, crawl_result.url
                    )
                    ...
                    if strategy_result and self._is_extraction_successful(strategy_result):
                        ...
                        if strategy_result:
                            logger.debug(f"Strategy {strategy.name} result validation failed - text length: {len(strategy_result.text)}, errors: {strategy_result.extraction_errors}")
