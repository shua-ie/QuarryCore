...
class ConfidenceScorer:
    ...
    async def calculate_confidence(
        ...
        scores['html_quality'] = await self._score_html_quality(
            extracted_content.text, original_html
        )
        
        # 6. Domain-specific score
        scores['domain_specific'] = await self._score_domain_specific(
            extracted_content, crawl_result
        )
    ...
    async def _score_html_quality(self, extracted_text: str, original_html: str) -> float:
        ...
        semantic_elements = ['article', 'section', 'main', 'header', 'footer', 'nav', 'aside']
        ...
        content_indicators = ['class="content"', 'class="article"', 'class="post"', 'id="content"']
        if any(indicator in html_lower for indicator in content_indicators):
            score += 0.1
        
        # Penalty for excessive script/style content
        script_style_ratio = (
            html_lower.count('<script') + html_lower.count('<style')
        ) / max(1, html_lower.count('<'))
    ...
    async def _score_domain_specific(
        ...
        if 'wikipedia.org' in domain:
            ...
            academic_terms = ['research', 'study', 'analysis', 'methodology', 'conclusion']
        ...
        spam_indicators = [
            'click here', 'buy now', 'free trial', 'limited time',
            'act now', 'call now', 'subscribe', 'sign up now'
        ]
    ...
    async def get_confidence_breakdown(
        ...
        breakdown['html_quality'] = await self._score_html_quality(
            extracted_content.text, original_html
        )
        breakdown['domain_specific'] = await self._score_domain_specific(
            extracted_content, crawl_result
        )
