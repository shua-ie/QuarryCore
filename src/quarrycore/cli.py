...
@click.pass_context
def run(ctx: click.Context, urls: Optional[Any], url: tuple[str, ...], batch_size: int, 
        max_concurrency: int, checkpoint_interval: int, resume_from: Optional[str], 
        dry_run: bool, interactive: bool) -> None:
...
async def run_standard_pipeline(pipeline: Pipeline, url_list: List[str], batch_size: int, 
                               checkpoint_interval: int, resume_from: Optional[Path]) -> None:
...
async def run_interactive_pipeline(pipeline: Pipeline, url_list: List[str], batch_size: int, 
                                  checkpoint_interval: int, resume_from: Optional[Path]) -> None:
...
@click.option('--format', 'output_format', default='json', 
              type=click.Choice(['json', 'yaml', 'table']), help='Output format')
@click.pass_context
...
@cli.command()
@click.option('--format', 'output_format', default='json',
              type=click.Choice(['json', 'prometheus']), help='Health check format')
