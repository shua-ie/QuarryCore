...
class AdaptiveCrawler:
    ...
    def _adapt_to_hardware(self, capabilities: HardwareCapabilities) -> None:
        ...
        self.adaptive_config.max_concurrent_per_domain = max(
            2, self.adaptive_config.max_concurrent_requests // 10
        )
    ...
    def _get_domain_semaphore(self, domain: str) -> asyncio.Semaphore:
        ...
        if domain not in self._domain_semaphores:
            self._domain_semaphores[domain] = asyncio.Semaphore(
                self.adaptive_config.max_concurrent_per_domain
            )
    ...
    async def _update_response_cache(
        self, 
        url: str, 
    ...
    async def _stream_large_response(self, response: httpx.Response) -> bytes:
        ...
        should_stream = (
            self.adaptive_config.stream_large_responses and
            content_length and
            int(content_length) > self.adaptive_config.stream_threshold_mb * 1024 * 1024
        )
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt < max_retries:
                        ...
                        logger.warning(
                            f"Retry {attempt + 1}/{max_retries} for {url} after {delay:.2f}s delay"
                        )
                    else:
    ...
    async def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_bytes_per_request": (
                self._total_bytes_downloaded / self._successful_requests
