...
class ParquetConfig(BaseModel):
    ...
    compression: Literal["snappy", "gzip", "brotli", "zstd"] = Field(default="snappy", description="Compression codec for Parquet files.")
    partition_by: list[str] = Field(default_factory=lambda: ["domain", "date"], description="Fields to partition data by.")
...
class RetentionConfig(BaseModel):
    ...
    archive_after_days: int | None = Field(default=90, description="Days after which to move warm data to cold storage. None to disable.")
    cold_storage_path: str = Field(default="./data/cold_store", description="Path for Zstandard-compressed archives.")
...
class BackupConfig(BaseModel):
    ...
    frequency_hours: int | None = Field(default=24, description="How often to perform backups in hours. None to disable.")
...
class DeduplicationConfig(BaseModel):
    ...
    domains: dict[DomainType, DomainQualityConfig] = Field(default_factory=dict, description="Domain-specific quality thresholds.")


class DomainQualityConfig(BaseModel):
    """Configuration for quality scoring for a specific domain."""
    min_overall_score: float = Field(default=0.7, ge=0, le=1, description="Minimum overall quality score to accept content.")
...
class ChunkingConfig(BaseModel):
    ...
    tokenizer_name: str = Field(default="mistralai/Mistral-7B-v0.1", description="HuggingFace tokenizer to use for chunking.")
...
class SamplingConfig(BaseModel):
    ...
    strategy: Literal["curriculum", "balanced", "random"] = Field(default="curriculum", description="Strategy for sampling and ordering data.")
    quality_weight: float = Field(default=0.7, description="Weight given to quality score during selection (0=no weight, 1=only quality).")
    rejection_sampling_factor: float = Field(default=2.0, description="How many candidates to consider for each final slot (higher means stricter quality selection).")
    domain_balance: dict[DomainType, float] | None = Field(default=None, description="Target proportions for each domain. If None, balance is equal.")

class FormattingConfig(BaseModel):
    """Configuration for output formatting."""
    format_type: Literal["instruction", "conversation", "document"] = Field(default="instruction", description="The format for the final training data.")
    instruction_template: str = Field(default="Summarize the following text:\n\n{text}", description="Template for generating instructions.")
...
class DatasetConfig(BaseModel):
    ...
    max_documents: int | None = Field(default=100000, description="Maximum number of documents to process for the dataset.")
...
class MonitoringConfig(BaseModel):
    ...
    log_file: str | None = Field(default="./logs/quarrycore.log", description="Path to log file. If None, logs to console.")
    prometheus_port: int | None = Field(default=9090, description="Port for Prometheus metrics exporter. None to disable.")
...
class Config(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_prefix="QUARRY_", env_nested_delimiter="__", case_sensitive=False
    )
