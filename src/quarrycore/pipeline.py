...
from quarrycore.recovery.dead_letter import FailedDocument
...
class Pipeline:
    ...
    def __init__(self, container: DependencyContainer, max_concurrency: int = 100) -> None:
        ...
        self.circuit_breakers = {
            stage.value: CircuitBreaker() for stage in PipelineStage
        }
            ...
            async with self.container.lifecycle():
                ...
                async with observability.start_monitoring():
                    ...
                    checkpoint_task = asyncio.create_task(
                        self._checkpoint_loop(checkpoint_interval)
                    )
        ...
        try:
            ...
            async with asyncio.TaskGroup() as tg:
                ...
                workers = [
                    ...
                    for i in range(min(10, len(self.state.urls_remaining)))
                ]
    ...
    async def _process_url(self, url: str, worker_id: str) -> ProcessingResult:
        ...
        try:
            ...
            try:
                quality_score = await quality.assess_quality(
