...
def _is_test_environment() -> bool:
    ...
    return any([
        ...
        any(frame.filename.endswith(('test_', '_test.py', '/tests/')) 
...
class RateLimitExceeded(HTTPException):
    ...
    def __init__(self, retry_after: int, limit: int = 0, remaining: int = 0, reset_time: float = 0):
        super().__init__(
...
class RedisRateLimiter:
    ...
    def __init__(
        ...
        if _is_test_environment():
            logger.info(f"Rate limiter initialized in TEST mode with {self.default_limit} requests per {self.window_seconds}s")
        else:
            logger.info(f"Rate limiter initialized in PRODUCTION mode with {self.default_limit} requests per {self.window_seconds}s")
        ...
        if self._redis_available and self.redis_client:
            try:
                return await self._check_rate_limit_redis(
                    identifier, limit, burst_limit, current_time
                )
            except Exception as e:
