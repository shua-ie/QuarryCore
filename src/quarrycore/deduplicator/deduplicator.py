...
class MultiLevelDeduplicator(DeduplicatorProtocol):
        ...
        *,
        ...
        if not is_new:
            ...
            if self.config.enable_all_levels:
                ...
                advanced_result = await self._check_advanced_levels(
                    doc_id, text, content_type
                )
                # Merge results
    ...
    async def _check_advanced_levels(
        self,
        doc_id: str,
        text: str,
        content_type: str
        ...
        print(f"DEBUG: About to check components")
    ...
    async def _check_semantic(
        ...
        if self.semantic is not None:
            similar_docs = await asyncio.to_thread(
                self.semantic.add, doc_id, text, content_type
            )
    ...
    async def _check_fuzzy(
        ...
        if self.fuzzy is not None:
            similar_docs = await asyncio.to_thread(
                self.fuzzy.add, doc_id, text, content_type
            )
