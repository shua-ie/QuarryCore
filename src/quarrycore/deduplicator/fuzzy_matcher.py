...
from rapidfuzz import fuzz, process, distance  # type: ignore[import-not-found]
...
class FuzzyMatcher:
            ...
            for match in matcher.get_matching_blocks():
                if match.size >= min_overlap:
                    ...
                    similarity = self._calculate_similarity(
                        query[match.a:match.a + match.size],
                        match_text
                    )
    ...
    def find_near_duplicates_batch(
        ...
        for i, (doc_id1, text1, type1) in enumerate(processed_docs):
            ...
            for j, (doc_id2, text2, type2) in enumerate(processed_docs[i+1:], i+1):
