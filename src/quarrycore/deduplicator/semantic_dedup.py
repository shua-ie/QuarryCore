...
class SemanticDeduplicator:
    ...
    def __init__(self, config: SemanticConfig, hardware: Optional[HardwareCapabilities] = None) -> None:
        self.config = config
    ...
    def compute_embedding(self, texts: List[str], show_progress: bool = False) -> Any:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            try:
                ...
                if HAS_TORCH and "out of memory" in str(e).lower():
                    ...
                    if batch_size > 1:
                        smaller_batch = self.compute_embedding(batch, show_progress=False)
    ...
    def add(self, doc_id: str, text: str, content_type: str = "general") -> List[Tuple[str, float]]:
        """
        ...
        similar_docs = self.find_similar_content(embedding, content_type, k=10)
    ...
    def _add_to_index(self, doc_id: str, embedding: Any) -> None:
        ...
        if self.index is not None and hasattr(self.index, "is_trained") and not self.index.is_trained:
            ...
            if len(self.embeddings_cache) >= self.config.nlist * 40:
                ...
                ids, vectors = zip(*self.embeddings_cache.items(), strict=False)
    ...
    def find_similar_content(self, embedding: Any, content_type: str, k: int = 10) -> List[Tuple[str, float]]:
        ...
        threshold = (
            self.config.content_type_thresholds.get(content_type, self.config.similarity_threshold)
            if self.config.content_type_thresholds
    ...
    def add_batch(self, documents: List[Tuple[str, str, str]]) -> Dict[str, List[Tuple[str, float]]]:
        """
        ...
        results = {}
        for doc_id, embedding, content_type in zip(doc_ids, embeddings, content_types, strict=False):
