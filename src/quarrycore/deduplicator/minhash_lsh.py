...
class MinHashLSHDeduplicator:
    ...
    def __init__(self, config: MinHashConfig, hardware: Optional[HardwareCapabilities] = None):
        self.config = config
    ...
    def _tokenize(self, text: str) -> List[str]:
        ...
        if self.config.tokenizer == "word":
            ...
            # Character n-grams
            tokens = [text[i : i + self.config.shingle_size] for i in range(len(text) - self.config.shingle_size + 1)]
        else:
    ...
    def save_index(self) -> None:
        ...
        data = {"lsh": self.lsh, "minhashes": self.minhashes, "config": self.config}
