...
class ShardedBloomFilter:
    ...
    def __init__(self, config: BloomFilterConfig, hardware: Optional[HardwareCapabilities] = None) -> None:
        self.config = config
    ...
    def save_all(self) -> None:
        ...
        with ThreadPoolExecutor(max_workers=min(4, self.config.num_shards)) as executor:
            futures = [executor.submit(self._save_shard, i) for i in range(self.config.num_shards)]
    ...
    def clear(self) -> None:
        ...
        for i in range(self.config.num_shards):
            with self.shard_locks[i]:
                if HAS_PYBLOOM:
                    shard_capacity = self.config.capacity // self.config.num_shards
                    self.shards[i] = BloomFilter(capacity=shard_capacity, error_rate=self.config.error_rate)
                else:
                    self.shards[i] = set()
