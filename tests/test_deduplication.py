...
from quarrycore.deduplicator import (
    ShardedBloomFilter, BloomFilterConfig,
    MinHashLSHDeduplicator, MinHashConfig,
    SemanticDeduplicator, SemanticConfig,
    FuzzyMatcher, FuzzyConfig,
    MultiLevelDeduplicator, DeduplicationConfig,
    DuplicateType
)
...
class TestBloomFilter:
    ...
    def test_bloom_filter_batch(self, test_dir):
        ...
        assert results[0][0] == True  # New
        assert results[1][0] == True  # New
        assert results[2][0] == False  # Duplicate
        assert results[3][0] == True  # New
...
class TestMinHashLSH:
    ...
    def test_minhash_batch_processing(self, test_dir):
        ...
        documents = [
            ...
            ("doc2", "Natural language processing allows computers to comprehend human language"),
            ("doc3", "Computer vision helps machines interpret visual information from images")
        ]
...
class TestSemanticDeduplication:
    ...
    def test_semantic_similarity(self, test_dir, mock_sentencetransformer):
        ...
        with patch('quarrycore.deduplicator.semantic_dedup.faiss') as mock_faiss:
            ...
            # Add documents
            similar1 = semantic.add("doc1", doc1, "general")
...
class TestFuzzyMatching:
    ...
    def test_fuzzy_batch_processing(self, test_dir):
        ...
        documents = [
            ...
            ("doc2", "Python is a high-level, interpreted programming language", "technical"),
...
class TestMultiLevelDeduplicator:
    ...
    async def test_near_duplicate_detection(self, dedup_config, hardware_caps):
        ...
        if not result2.is_duplicate:
            print(f"DEBUG: Not detected as duplicate")
