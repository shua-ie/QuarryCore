...
from quarrycore.crawler import (
    AdaptiveCrawler, CircuitBreaker, DomainRateLimiter, 
    RobotsCache, UserAgentRotator
)
from quarrycore.crawler.adaptive_crawler import ConnectionPoolConfig, AdaptiveConfig
from quarrycore.protocols import (
    CrawlResult, HardwareCapabilities, HardwareType, 
    ProcessingStatus, ErrorInfo, ErrorSeverity
)
    ...
    async def test_robots_txt_compliance(self, mock_httpx_client):
        ...
        with patch('httpx.AsyncClient', return_value=mock_httpx_client):
            ...
            async with crawler:
                ...
                assert hasattr(result, 'robots_allowed')
                assert result.status in [ProcessingStatus.SKIPPED, ProcessingStatus.FAILED, ProcessingStatus.COMPLETED]
...
class TestCrawlerPerformance:
    ...
    @pytest.mark.performance
    @pytest.mark.skipif(True, reason="Memory benchmark - skipped in test environment to avoid resource conflicts")
