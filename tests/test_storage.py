...
from quarrycore.config import StorageConfig, SQLiteConfig, ParquetConfig, RetentionConfig, BackupConfig
from quarrycore.storage import StorageManager, SQLiteManager, ParquetStore
from quarrycore.protocols import (
    ExtractedContent, ContentMetadata, QualityScore, DuplicationResult,
    DomainType, ContentType
)
...
class TestStorageManager:
    ...
    def sample_extracted_content(self):
        ...
        return ExtractedContent(
            ...
            text="This comprehensive guide covers advanced software engineering principles including architecture patterns, performance optimization, and quality assurance methodologies.",
            language="en",
            word_count=23,
            tables=[],
            images=[],
            links=[
                {"url": "https://example.com/patterns", "text": "Architecture Patterns"},
                {"url": "https://example.com/optimization", "text": "Performance Optimization"}
            ],
            code_blocks=[],
    ...
    async def test_storage_cleanup_and_isolation(self, temp_dir):
        ...
        # Store some data
        document = {
            "text": "Isolation test content",
            "metadata": {"title": "Isolation Test", "source": "test.com"}
        }
        doc_id = await manager.store_document(document)
