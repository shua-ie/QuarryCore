...
from quarrycore.config import SQLiteConfig, ParquetConfig
from quarrycore.storage import ParquetStore, SQLiteManager
from quarrycore.web.main import app
from quarrycore.protocols import ExtractedContent, ContentMetadata, CrawlResult, ProcessingStatus
from quarrycore.metadata.metadata_extractor import MetadataExtractor
...
class TestAPIAuthentication:
    ...
    async def test_api_endpoint_authentication(self):
        ...
        for endpoint in sensitive_endpoints:
            ...
            assert response.status_code in [401, 403, 404, 422]  # Include 422 for unprocessable entity
    ...
    async def test_rate_limiting_protection(self):
        ...
        assert any(code == 429 for code in status_codes) or \
               any(code >= 400 for code in status_codes[-20:])  # Last 20 should show limiting
...
class TestDataPrivacyCompliance:
    ...
    async def test_gdpr_data_deletion(self, temp_dir):
        ...
        from quarrycore.protocols import ExtractedContent, ContentMetadata, QualityScore, DuplicationResult
    ...
    async def test_consent_management(self):
        ...
        with patch('quarrycore.crawler.robots_parser.RobotsCache.is_allowed', new_callable=AsyncMock) as mock_is_allowed:
        ...
        if config.log_file and config.log_file.exists():
            ...
            if len(log_content) > 0:
                has_security_logging = any(
                    term in log_content.lower() 
                    for term in ['security', 'authentication', 'event', 'error', 'info']
                )
    ...
    async def test_security_headers_compliance(self):
        ...
        for header, expected_value in security_headers.items():
            if header in response.headers:
                ...
                assert any(secure_part in actual_value 
                          for secure_part in expected_value.split()) 